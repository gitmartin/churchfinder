...
try {
  ...
  await page.getByText("Control connected · SSE height updates").waitFor();
  const frame = page.frameLocator("#faith-widget iframe");
  const frameElement = page.locator("#faith-widget iframe");
  ...
  await page.waitForFunction(initial => parseFloat(document.querySelector("#faith-widget iframe").style.height) > initial + 80, initialHeight);
  ...
  await page.waitForFunction(previous => parseFloat(document.querySelector("#faith-widget iframe").style.height) > previous + 60, wideHeight);
  ...
  await page.waitForFunction(initial => Math.abs(parseFloat(document.querySelector("#faith-widget iframe").style.height) - initial) <= 2, initialHeight);
