...
$testAssembly = Join-Path $repoRoot 'artifacts/bin/Faith.UI.TestSite/debug/Faith.UI.TestSite.dll'
if (-not (Test-Path -LiteralPath $webAssembly) -or -not (Test-Path -LiteralPath $testAssembly)) { & "$PSScriptRoot/build.ps1" -Target All }
...
try {
  ...
  $hostStart = [System.Diagnostics.ProcessStartInfo]::new((Get-Command dotnet).Source)
  $hostStart.WorkingDirectory = Join-Path $repoRoot 'src/Faith.UI.TestSite'
  $hostStart.UseShellExecute = $false
  $hostStart.CreateNoWindow = $true
  $hostStart.ArgumentList.Add($testAssembly)
  $hostStart.Environment['ASPNETCORE_ENVIRONMENT'] = 'Development'
  $hostStart.Environment['ASPNETCORE_URLS'] = 'http://localhost:5242'
