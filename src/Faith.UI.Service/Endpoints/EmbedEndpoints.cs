...
public static class EmbedEndpoints
{
  public static void UseFaithEmbeds(this WebApplication app)
  {
    ...
    app.Map("/embed", branch => branch.Run(async context =>
    {
      ...
      string component = context.Request.Path.Value?.Trim('/') ?? "";
      bool isResponse = component == "api/lorem";
      if (!isResponse && component is not ("a" or "b" or "c" or "lorem"))
      {
      ...
      string connectionId = query["connectionId"].ToString();
      string resizeToken = query["resizeToken"].ToString();
      string componentKey = component == "lorem" ? "a" : component;
      if ((parentOrigin.Length > 0 || connectionId.Length > 0 || resizeToken.Length > 0)
        && !context.RequestServices.GetRequiredService<EmbedConnectionRegistry>()
          .TryGetForFrame(connectionId, resizeToken, parentOrigin, componentKey))
      {
