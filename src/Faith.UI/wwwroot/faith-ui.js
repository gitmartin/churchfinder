(() => {
  ...
  function mount(target, options = {}) {
    ...
    frame.style.cssText = `display:block;width:100%;height:${initialHeight}px;border:0;visibility:hidden;`;
    frame.setAttribute("aria-hidden", "true");
    frame.referrerPolicy = "no-referrer";
    frame.setAttribute("sandbox", "allow-scripts allow-same-origin");
    const surface = document.createElement("div");
    surface.className = "faith-ui-embed";
    surface.style.position = "relative";
    surface.setAttribute("aria-busy", "true");
    const loading = document.createElement("div");
    loading.className = "faith-ui-loading";
    loading.setAttribute("role", "status");
    loading.style.cssText = "position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:24px;color:#365b45;font:500 14px/1.5 system-ui,sans-serif;text-align:center;";
    const icon = document.createElement("span");
    icon.setAttribute("aria-hidden", "true");
    icon.style.cssText = "display:block;width:32px;height:32px;box-sizing:border-box;border:3px solid #d8e3db;border-top-color:#174c3b;border-radius:50%;";
    const label = document.createElement("span");
    label.textContent = `Loading component ${component.toUpperCase()}…`;
    loading.append(icon, label);
    const spinner = window.matchMedia("(prefers-reduced-motion: reduce)").matches ? null
      : icon.animate([{ transform: "rotate(0deg)" }, { transform: "rotate(360deg)" }], { duration: 900, iterations: Infinity });
    surface.append(frame, loading);
